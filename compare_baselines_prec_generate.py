                    ...
                    for m in m_vals:
                        for n in n_vals:
                            for (attr_type, attr, freq) in zip(selected_attrs['attributeType'], selected_attrs['attribute'], selected_attrs['freq']):
                                if (2 * n <= freq):
                                    subprocess.check_call(['python3', 'compare_baselines_prec.py', '-a', attr, '-t', attr_type, '-n', str(n), '-m', str(m), '-e', embedding, '--sim', sim, '-d', str(delta), '-k', str(k), '-s' if sphere else '', '-v', '-N', str(N)])