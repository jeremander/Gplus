            ...
            for n in n_vals:
                for (attr, attr_type) in zip(selected_attrs['attribute'], selected_attrs['attributeType']):
                    if (selected_attrs[(selected_attrs['attribute'] == attr) & (selected_attrs['attributeType'] == attr_type)]['freq'].iloc[0] >= 2 * n):
                        ...
                        cmd = 'qsub -q all.q -l num_proc=%d,mem_free=4G,h_rt=24:00:00 -b Y -V -cwd -j yes -o . -N baseline3%s_%s_n%d_%s_k%d%s "%s"' % (jobs, safe_attr, attr_type, n, embedding, k, '_normalize' if sphere else '', subcmd)
                        print(cmd)
