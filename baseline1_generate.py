    ...
    for n in n_vals:
        for (attr, attr_type) in zip(selected_attrs['attribute'], selected_attrs['attributeType']):
            if (selected_attrs[(selected_attrs['attribute'] == attr) & (selected_attrs['attributeType'] == attr_type)]['freq'].iloc[0] >= 2 * n):
                csv_filename = folder + '%s_%s_n%d_m%d_precision.csv' % (attr_type, attr, n, max_count_features)
                plot_filename = folder + '%s_%s_n%d_m%d_precision.png' % (attr_type, attr, n, max_count_features)
                if overwrite:
                ...
                if (write or (plot and os.path.isfile(csv_filename) and (not os.path.isfile(plot_filename)))):
                    if grid:
                        ...
                        cmd = 'qsub -q all.q -l num_proc=%d,mem_free=%dG,h_rt=72:00:00 -b Y -V -cwd -j yes -o . -N baseline1_%s_%s_n%d_m%d "%s"' % (jobs, memory, safe_attr, attr_type, n, max_count_features, subcmd)
                        print(cmd)
                        subprocess.Popen(cmd, shell = True, stdout = subprocess.PIPE)
                    else:
                        subprocess.check_call(['python3', 'baseline1.py', '-a', attr, '-t', attr_type, '-n', str(n), '-m', str(max_count_features), '-S', str(num_samples), '-v' if plot else '', '-j', str(jobs)])