                    ...
                    if (selected_attrs[(selected_attrs['attribute'] == attr) & (selected_attrs['attributeType'] == attr_type)]['freq'].iloc[0] >= 2 * n):
                        ...
                        if (write or (plot and os.path.isfile(csv_filename) and (not os.path.isfile(plot_filename)))):
                            if grid:
                                ...
                                cmd = 'qsub -q all.q -l num_proc=%d,mem_free=%dG,h_rt=24:00:00 -b Y -V -cwd -j yes -o . -N baseline2_%s_%s_n%d_%s_k%d%s "%s"' % (jobs, memory, safe_attr, attr_type, n, embedding, k, '_normalize' if sphere else '', subcmd)
                                print(cmd)
                                subprocess.Popen(cmd, shell = True, stdout = subprocess.PIPE)
                            else:
                                subprocess.check_call(['python3', 'baseline2.py', '-a', attr, '-t', attr_type, '-n', str(n), '-e', embedding, '-k', str(k), '-S', str(num_samples), '-v' if plot else '', '-j', str(jobs)] + (['-s'] if sphere else []))