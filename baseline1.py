# Performs vertex nomination using random forests and logistic regression. Obscures one attribute type and does binary classification on the desired attribute of that type. Averages precision-by-rank over a number of samples.
...
def main():
    ...
    attr, attr_type, num_train_each, max_count_features, num_estimators, num_samples, jobs = opts.attr, opts.attr_type, opts.num_train_each, opts.max_count_features, opts.num_estimators, opts.num_samples, opts.jobs

    folder = 'gplus0_lcc/baseline1/'
    agg_precision_filename = folder + '%s_%s_n%d_m%d_e%d_precision.csv' % (attr_type, attr, num_train_each, max_count_features, num_estimators)
    agg_feature_importances_filename = folder + '%s_%s_n%d_m%d_e%d_feature_importances.csv' % (attr_type, attr, num_train_each, max_count_features, num_estimators)
    agg_word_importances_filename = folder + '%s_%s_n%d_m%d_e%d_word_importances.csv' % (attr_type, attr, num_train_each, max_count_features, num_estimators)
    plot_filename = folder + '%s_%s_n%d_m%d_e%d_precision.png' % (attr_type, attr, num_train_each, max_count_features, num_estimators)
    ...
    #plt.legend(handles = [rfc_scatter, logreg_scatter, guess])
